...
private:
  ...
  // generate decay
  const Event *genDecay(int id, double mass, double life, double energy,
                        double bCos, double bPhi, double mPos, double mNeg);
  ...
  // uniform
  static double randF() { return random() * 1.0 / RAND_MAX; }
  static double randF(float max) { return random() * max / RAND_MAX; }
  ...
  static double randE(float c) { return -c * log(randF()); }

  // gaussian
  static double randG() { return sqrt(-log(randF())) * sin(randPhi()); }
  static double randG(float sigma) {
    return sigma * sqrt(-log(randF())) * sin(randPhi());
  }
  static double randG(float mean, float sigma) {
    return mean + (sigma * sqrt(-log(randF())) * sin(randPhi()));
  }

  // positive gaussian
  static double randH() { return sqrt(-log(randF())) * sin(randF(M_PI)); }
  ...
  // cosine complement
  static float complement(float c, float f) { return 1.0 - ((1.0 - c) * f); }
  ...
  public:
    ...
    Vector3() : xv(0.0), yv(0.0), zv(0.0), ms(-1.0), mv(-1.0) {}

    // create point / momentum with coordinates / components "x,y,z"
    Vector3(double x, double y, double z)
        : xv(x), yv(y), zv(z), ms(-1.0), mv(-1.0) {}
    ...
    // reverse vector
    Vector3 operator-() const { return Vector3(-xv, -yv, -zv); }
    ...
    Vector3 operator/(double f) const { return *this * (1.0 / f); }
    ...
    double operator%(const Vector3 &v) const { return (*this | v) / v.mod(); }

    // vector product
    Vector3 operator^(const Vector3 &v) const {
      return Vector3((yv * v.zv) - (zv * v.yv), (zv * v.xv) - (xv * v.zv),
                     (xv * v.yv) - (yv * v.xv));
    }
    ...
    Vector3 operator&&(const Vector3 &v) const { return *this - (*this || v); }
  ...
  public:
    ...
    Vector4(const Vector3 &pv, double im) : vp(pv), pm(im) { ee = -1.0; }
    ...
    double beta() const { return vp.mod() / energy(); }
    double gamma() const { return energy() / pm; }
    ...
    Vector4 boost(const Vector4 &v) {
      ...
      Vector3 t = vp - u;
      return Vector4(
          v.p().norm(((vp % v.p()) + (v.beta() * energy())) * v.gamma()) + t,
          pm);
    }
