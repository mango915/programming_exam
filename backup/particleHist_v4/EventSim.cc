...
EventSim::EventSim(unsigned int n, unsigned int s) : nMax(n), evId(0) {
  srandom(s);
}
...
const Event *EventSim::genK0(int id) {
  ...
  // energy
  double energy = randE(meanEnergyK0, rmsEnergyK0, minEnergyK0);

  // decay
  return genDecay(id, massK0, lifeK0, energy, cosBeam, phiBeam, massPion,
                  massPion);
}
...
const Event *EventSim::genLambda0(int id) {
  ...
  // energy
  double energy = randE(meanEnergyLambda0, rmsEnergyLambda0, minEnergyLambda0);

  // decay
  return genDecay(id, massLambda0, lifeLambda0, energy, cosBeam, phiBeam,
                  massProton, massPion);
}
...
// generate decay
const Event *EventSim::genDecay(int id, double mass, double life, double energy,
                                double bCos, double bPhi, double mPos,
                                double mNeg) {
  ...
  Vector4 p0(pb * bSin * cos(bPhi), pb * bSin * sin(bPhi), pb * bCos, mass);
  ...
  double pd =
      sqrt((mass * mass / 4.0) +
           ((pow(mPos, 4) + pow(mNeg, 4) - ((2 * mPos * mPos * mNeg * mNeg) +
  ...
  Vector3 pPos = Vector4(randG(px, px * px * fRMS), randG(py, py * py * fRMS),
                         randG(pz, pz * pz * fRMS), mPos)
                     .boost(p0)
                     .p();
  Vector3 pNeg = Vector4(randG(-px, px * px * fRMS), randG(-py, py * py * fRMS),
                         randG(-pz, pz * pz * fRMS), mNeg)
                     .boost(p0)
                     .p();
  Vector3 *pDaug1;
  Vector3 *pDaug2;
...
void EventSim::addBG(Event *ev) {
  ...
  double energy = randE(meanEnergyBG, rmsEnergyBG, minEnergyBG);
  int charge = randC();
  ev->add(energy * sinPart * cos(phiPart), energy * sinPart * sin(phiPart),
          energy * cosPart, charge);
  return;
}
